...
import { Ellipse, Group, Image, Line, Path, Rect, Text } from 'leafer-ui';
...
export const useCanvasStore = defineStore('canvas', {
  ...
  actions: {
    ...
    updateZoom(delta: number) {
      const newZoom = Math.max(canvasConfig.zoom.min, Math.min(canvasConfig.zoom.max, this.zoom + delta));
      this.zoom = newZoom;
    ...
    createElementFromData(tag: string, data: Record<string, unknown>): LeaferElement | null {
      try {
        switch (tag) {
          ...
          case 'Group':
            return new Group(data as ConstructorParameters<typeof Group>[0]);
          default:
