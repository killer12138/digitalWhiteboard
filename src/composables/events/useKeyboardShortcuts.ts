...
export function useKeyboardShortcuts(
  ...
  const { groupElements, ungroupElements, canGroup, canUngroup } = useGroupTool();
  ...
  function buildShortcutKeyString(parsedShortcut: ParsedShortcut): string {
    ...
    return modifierKeys.length > 0 ? `${modifierKeys.join('+')}+${parsedShortcut.key}` : parsedShortcut.key;
  }
  ...
  function handleGroupShortcut(event: KeyboardEvent): boolean {
    const isCtrlOrMeta = event.ctrlKey || event.metaKey;
    const isGroupKey = event.code === 'KeyG';

    if (!isCtrlOrMeta || !isGroupKey) {
      return false;
    }
  ...
  async function processMatchedShortcut(pluginId: string, toolType: ShortcutActionType): Promise<boolean> {
