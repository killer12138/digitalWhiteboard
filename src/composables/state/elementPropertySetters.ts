...
class ImageElementPropertySetter implements ElementPropertySetter {
  setProperties(props: ElementProps | undefined, refs: ElementPropertyRefs): void {
    if (refs.imageWidth) {
      refs.imageWidth.value = props?.imageWidth ?? 100;
    }
    if (refs.imageHeight) {
      refs.imageHeight.value = props?.imageHeight ?? 100;
    }
  }
}
...
export const imageElementPropertySetter = new ImageElementPropertySetter();
...
export const elementPropertySetterMap: Record<string, ElementPropertySetter> = {
  ...
  image: imageElementPropertySetter
};
