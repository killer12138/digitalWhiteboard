...
export function useElementPopover() {
  ...
  const canBringForward = computed(() => {
    return layerControl.canBringForward(selectedElement.value);
  });

  const canSendBackward = computed(() => {
    return layerControl.canSendBackward(selectedElement.value);
  });

  function bringElementForward() {
    if (selectedElement.value) {
      layerControl.bringForward(selectedElement.value);
    }
  }

  function sendElementBackward() {
    if (selectedElement.value) {
      layerControl.sendBackward(selectedElement.value);
    }
  }

  function bringElementToFront() {
    if (selectedElement.value) {
      layerControl.bringToFront(selectedElement.value);
    }
  }

  function sendElementToBack() {
    if (selectedElement.value) {
      layerControl.sendToBack(selectedElement.value);
    }
  }
